    ...
    <div className="flex min-w-[320px] flex-1 flex-col">
      ...
      <div className="flex-1 overflow-y-auto p-4 text-[11px] text-maestro-text">
        <dl className="grid grid-cols-[110px_1fr] gap-x-3 gap-y-1">
          <dt className="text-maestro-muted">Driver</dt>
          <dd>{job.driver}</dd>
          <dt className="text-maestro-muted">Scope</dt>
          <dd>
            {job.scope}
            {job.projectHash ? ` (${job.projectHash.slice(0, 8)})` : ""}
          </dd>
          <dt className="text-maestro-muted">Schedule</dt>
          <dd className="font-mono">{job.schedule ?? "—"}</dd>
          <dt className="text-maestro-muted">Enabled</dt>
          <dd>{job.enabled ? "yes" : "no"}</dd>
          {job.driver === "maestro" && job.maestro && (
            <>
              <dt className="text-maestro-muted">Command</dt>
              <dd className="font-mono">{job.maestro.command}</dd>
              <dt className="text-maestro-muted">Args</dt>
              <dd className="font-mono">{job.maestro.args.join(" ")}</dd>
              {job.maestro.cwd && (
                <>
                  <dt className="text-maestro-muted">Cwd</dt>
                  <dd className="font-mono">{job.maestro.cwd}</dd>
                </>
              )}
            </>
          )}
          {job.driver === "claude-trigger" && job.claudeTrigger && (
            <>
              <dt className="text-maestro-muted">Trigger ID</dt>
              <dd className="font-mono text-[10.5px]">{job.claudeTrigger.triggerId ?? "—"}</dd>
              <dt className="text-maestro-muted">Prompt</dt>
              <dd className="whitespace-pre-wrap">{job.claudeTrigger.prompt}</dd>
              <dt className="text-maestro-muted">Connectors</dt>
              <dd>{job.claudeTrigger.mcpConnectors.join(", ") || "—"}</dd>
            </>
          )}
        </dl>
